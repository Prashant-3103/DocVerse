...
import { FaFileUpload, FaGoogleDrive, FaImage, FaCheckCircle } from "react-icons/fa";
...
export default function FileUpload() {
  ...
  const handleUploadClick = async () => {
    ...
    if (uploadMethod === "file" || uploadMethod === "image") {
  ...
  return (
    <div className="flex flex-col items-center p-4 space-y-4 bg-gray-100 rounded-lg shadow-lg max-w-lg mx-auto w-full">
      <h2 className="text-lg font-bold text-gray-700 text-center">Upload a File</h2>

      <div className="w-full">
        <label className="block text-sm font-medium text-gray-700 mb-2 text-center">Choose Upload Method:</label>
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <button
            ...
            className={`flex items-center px-4 py-2 rounded-lg shadow-md text-sm ${
              uploadMethod === "image" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"
            }`}
          >
            <FaImage className="mr-2" /> Image Upload
          </button>
        </div>
      </div>
